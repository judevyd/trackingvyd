      ...
      <v-data-table :headers="headers" :items="dataItems" :search="search" class="elevation-1" >
        <template v-slot:top>
         ...
         <template v-slot:item.jofaction="{ item }">
            <div class="my-2">
              <v-btn depressed small color="primary" @click="jofActions(item)">MOVE TO DISPATCHING</v-btn>
...
<script>
  export default {
    data: () => ({
      ...
      // Status:['Mold Section','Plastic Section','Wax Section','Casting Section','Salugar Section','Stone Section'],
